...
The `home-routes` handler in the `photo-api.routes.home` namespace
...
The routes are aggregated and wrapped with middleware in the `photo-api.handler` namespace:
...
Request middleware functions are located under the `photo-api.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `photo-api.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `photo-api.core` in some other namespace.
